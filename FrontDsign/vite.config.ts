...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      VitePWA({
        // manifest.webmanifest and its <link> are hand-written in index.html
        // (inside the figma:head-start/end slot) so this plugin only builds
        // the service worker; it must not also inject a competing manifest link.
        manifest: false,
        injectRegister: null,
